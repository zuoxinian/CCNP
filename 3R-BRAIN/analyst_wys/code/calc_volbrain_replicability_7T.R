...
volbrain_ibp7t <- data_df %>% filter(scanner=='IBP_Magnetom')


# orginaze dataframe
## right hemi
df_names <- colnames(volbrain_ibp7t)
...
for (i in 1:nmeasures){
  ...
  L_metric <- volbrain_ibp7t[,L_df_names[i]]
  R_metric <- volbrain_ibp7t[,R_df_names[i]]
  #paired t-test
...
fout <- paste0(workdir,'results/','replicability/','volbrain_ibp7t_LR_diff_by_wys.csv')
write.csv(df_lr,fout)
####################### gender differences########################################
#calculat AI
cal_AI <- function(L,R){((L-R)/(L+R))*2} #define lr asymmetry function

volbrain_ibp7t$gender <- as.factor(volbrain_ibp7t$gender)
ICV <- volbrain_ibp7t$Tissue.IC.cm3
volbrain_ibp7t$cICV <- scale(ICV,center=T)

#cal gender and age diff
gender_distribute_diff <- chisq.test(summary(volbrain_ibp7t$gender))
volbrain_ibp7t_male <- volbrain_ibp7t %>% filter(gender=='1')
volbrain_ibp7t_female <- volbrain_ibp7t %>% filter(gender=='0')
age_diff <- t.test(volbrain_ibp7t_male$age,volbrain_ibp7t_female$age)
education_diff <- t.test(volbrain_ibp7t_male$education,volbrain_ibp7t_female$education)
...
fout <-  paste0(workdir,'results/','replicability/','gender_match_test-7T.csv')
write.csv(matched_test,fout)

#loop each metric calculate the residual p value after regressed age and icv
idx_male <- which(volbrain_ibp7t['gender']=='1')
idx_female <- which(volbrain_ibp7t['gender']=='0')
...
for (k in 1:nmeasures){
  ...
  metric_AI <- cal_AI(volbrain_ibp7t[,L_df_names[k]],volbrain_ibp7t[,R_df_names[k]])
  tmp_df <- data.frame(metric_AI=metric_AI,cICV=volbrain_ibp7t$cICV)
...
fout <- paste0(workdir,'results/','replicability/','volbrain_ibp7t_gender-diff_by_wys.csv')
